...
import Sidebar, { CategoriaCatalogo, categoriasDe, subcategoriasDe } from "@/components/Sidebar";
import { getProdutos } from "@/lib/produtos";
import { getEmpresaAtual, Empresa } from "@/lib/empresa";
import { useCarrinho } from "@/lib/carrinho-context";
import { Produto, HorariosLoja, lojaAberta, rotuloSubcategoria } from "@/lib/types";

function ePromocao(p: Produto) { return p.precoPromocional != null && p.preco != null && p.precoPromocional < p.preco; }

export default function Home() {
  const [categoriaAtiva, setCategoriaAtiva] = useState<CategoriaCatalogo>("");
  ...
  const horarios = empresa?.configuracoes && typeof empresa.configuracoes === "object" ? (empresa.configuracoes.horarios as HorariosLoja | undefined) : undefined;
  const aberta = lojaAberta(horarios);

  useEffect(() => {
    Promise.all([getEmpresaAtual(), getProdutos()]).then(([emp, lista]) => {
      setEmpresa(emp);
      setProdutos(lista);
      const primeira = categoriasDe(lista).find((c) => c.id !== "promocoes");
      setCategoriaAtiva(primeira?.id ?? "");
  ...
  const produtosFiltrados = useMemo(() => {
    ...
    if (ordenacao === "alfabetica") return [...filtrados].sort((a,b)=>a.nome.localeCompare(b.nome,"pt-BR"));
    if (ordenacao === "aleatoria") return [...filtrados].sort(()=>Math.random()-0.5);
      ...
      <div className="mx-auto flex max-w-6xl gap-6 px-4 md:px-6">
        ...
        <div className="min-w-0 flex-1">
          <nav className="sticky top-0 z-10 -mx-4 flex min-w-0 gap-2 overflow-x-auto bg-neutral-50 px-4 py-3 lg:hidden">
            {categoriasComProduto.map((c)=><button key={c.id} onClick={()=>selecionarCategoria(c.id)} className={`shrink-0 whitespace-nowrap rounded-full px-3 py-1.5 text-sm ${categoriaAtiva===c.id&&!emBusca?"bg-brand-600 text-white":"border border-neutral-200 bg-white text-neutral-600"}`}>{c.label}</button>)}
          </nav>
          {subsDaCategoria.length>0&&!emBusca&&<div className="-mx-4 mb-2 flex min-w-0 gap-2 overflow-x-auto px-4 pb-1 lg:hidden"><button onClick={()=>setSubcategoriaAtiva(null)} className={`shrink-0 whitespace-nowrap rounded-full px-3 py-1.5 text-xs ${subcategoriaAtiva===null?"bg-brand-50 font-medium text-brand-700":"text-neutral-500"}`}>Todos</button>{subsDaCategoria.map((s)=><button key={s} onClick={()=>setSubcategoriaAtiva(s)} className={`shrink-0 whitespace-nowrap rounded-full px-3 py-1.5 text-xs ${subcategoriaAtiva===s?"bg-brand-50 font-medium text-brand-700":"text-neutral-500"}`}>{rotuloSubcategoria(s)}</button>)}</div>}
          <div className="mb-3 flex justify-end"><select value={ordenacao} onChange={(e)=>setOrdenacao(e.target.value as typeof ordenacao)} className="rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm text-neutral-700 outline-none focus:border-brand-500"><option value="cadastro">Ordem de cadastro</option><option value="aleatoria">Ordem aleatória</option><option value="alfabetica">Ordem alfabética</option></select></div>
