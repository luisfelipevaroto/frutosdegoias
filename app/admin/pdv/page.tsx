...
export default function PDV() {
  ...
  return <main className="min-h-screen bg-neutral-50 p-4"><div className="mx-auto max-w-7xl">
    ...
    {produtoModal && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center" onClick={() => setProdutoModal(null)}><div className="w-full max-w-md rounded-2xl bg-white p-5" onClick={(e) => e.stopPropagation()}><div className="flex justify-between"><div><h2 className="text-lg font-bold">{produtoModal.nome}</h2><p className="text-xs text-neutral-500">Configure o item</p></div><button onClick={() => setProdutoModal(null)} className="text-xl">×</button></div>{produtoModal.variacoes?.length > 0 && <div className="mt-4"><p className="mb-2 text-sm font-semibold">Variação</p><div className="grid gap-2">{produtoModal.variacoes.map((v) => <button key={v.id} onClick={() => setVariacaoModal(v)} className={`flex justify-between rounded-lg border p-3 text-sm ${variacaoModal?.id === v.id ? "border-brand-600 bg-brand-50" : ""}`}><span>{v.nome}</span><b>{brl(Number(v.preco))}</b></button>)}</div></div>}{produtoModal.adicionais.length > 0 && <div className="mt-4"><p className="mb-2 text-sm font-semibold">Adicionais</p><div className="grid gap-2">{produtoModal.adicionais.map((a) => <label key={a.id} className="flex cursor-pointer justify-between rounded-lg border p-3 text-sm"><span><input type="checkbox" checked={adicionaisModal.some((x) => x.id === a.id)} onChange={() => alternarAdicional(a)} className="mr-2" />{a.nome}</span><b>+ {brl(Number(a.preco))}</b></label>)}</div></div>}<button onClick={confirmarProduto} className="mt-5 w-full rounded-lg bg-brand-700 p-3 font-bold text-white">Adicionar ao pedido</button></div></div>}
  </div></main>;
}
